...
export const outgoingRepo = {
  async list(opts: {
    ...
    const conditions: SQL[] = [eq(outgoingGoods.outletId, opts.outletId)];
  ...
  async create(data: typeof outgoingGoods.$inferInsert, tx: DbOrTx = db) {
    const [row] = await tx.insert(outgoingGoods).values(data).returning();
    return row!;
  },

  async update(id: string, data: Partial<typeof outgoingGoods.$inferInsert>, tx: DbOrTx = db) {
    const [row] = await tx.update(outgoingGoods).set(data).where(eq(outgoingGoods.id, id)).returning();
    if (!row) throw new NotFoundError(`Outgoing goods ${id} not found`);
    return row;
  },
};