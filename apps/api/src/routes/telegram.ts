...
export const telegramRoutes = new Elysia({ prefix: '/api/telegram', tags: ['Telegram'] })
  ...
  .post('/link', async ({ jwt, headers }: any) => {
  ...
  // Get link status
  .get('/status', async ({ jwt, headers }: any) => {
  ...
  .get('/queue/stats', async ({ jwt, headers }: any) => {
  ...
  .get('/queue', async ({ jwt, query, headers }: any) => {
    await requireRole(await getAuthUser(jwt, headers), ['manager']);
